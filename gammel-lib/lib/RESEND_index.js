...
eventBus.on("text", async (fromAddress, message) => {
	...
	case "resend":
	    ...
	    walletDefinedByAddresses.sendSharedAddressToPeer("0MMSXREGDGX4ZHX2NS23TLXX4V3VY5YG4","AGTHYLJJG4T6XIXALSJC4UXVOMMFNZKW", (err) => {
		    if (err) device.sendMessageToDevice(fromAddress,"test","Something went wrong: " + err);
		    return true;
	    });
	    walletDefinedByAddresses.sendSharedAddressToPeer("0PZMD4GQWXEEAEYOMU2PHHFTAYYHLX636","EGNRF5ID5IPIHO2Z2IR6UTMF6UATIFYE", (err) => {
		    if (err) device.sendMessageToDevice(fromAddress,"test","Something went wrong: " + err);
		    return true;
	    });
	    walletDefinedByAddresses.sendSharedAddressToPeer("0MMSXREGDGX4ZHX2NS23TLXX4V3VY5YG4","URVKVEENI5F3SLSKFJZYL6T5UITWYTMC", (err) => {
		    if (err) device.sendMessageToDevice(fromAddress,"test","Something went wrong: " + err);
		    return true;
	    });
	    walletDefinedByAddresses.sendSharedAddressToPeer("0PZMD4GQWXEEAEYOMU2PHHFTAYYHLX636","N22LT66TYTUH3YMGEC6GWH4VQS6SG323", (err) => {
		    if (err) device.sendMessageToDevice(fromAddress,"test","Something went wrong: " + err);
		    return true;
	    });
	    device.sendMessageToDevice("0PZMD4GQWXEEAEYOMU2PHHFTAYYHLX636", "text", "The smart contracts was attempted re-broadcast to you");
	    break;
