...
export async function withdrawToPollopollo(sharedAddress: string) {
    ...
    let contract = await getContractBySharedAddress(sharedAddress);
    if (!contract) {
        ...
        /* The wallet address and device address below is the PolloPollo "management" address.
           ...
           If a user requested the withdrawal, they will be sent to that address and if not, it will be sent to the public PolloPollo donation
           wallet to be used for other donations.
        */
        await state.wallet.sendAllBytesFromAddress(contract.SharedAddress, "FWEZQDWNKABOEENRENJDYIIWMCUGYU3R", "0QZMFST5OJ4YS53Z2LMLHW2PVQUI4ZHS3", async (err, unit) => {
            if (err) {
